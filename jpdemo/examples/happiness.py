...
def happiness_plot(request):
    ...
    bg = jp.QBtnToggle(push=True, glossy=True, toggle_color='primary', value='', a=d, input=stack_change, style='margin-right: 30px')
    bg.options = [
    ...
    wp.toggle_list = []
    corr_button = jp.QBtn(label='Show pairwise correlation', a=d, click=corr_button_click)
    for i in df.columns[3:]:
        wp.toggle_list.append(jp.QToggle(checked_icon='check', color='green', unchecked_icon='clear', value=True,
                                         label=f'{i}', a=d,  input=series_change)) # ,style='margin-right: 10px',
    chart = df.jp.plot('Country', wp.cols_to_plot, kind='column', a=wp, title='World Happiness Ranking',
                   subtitle='Click and drag in the plot area to zoom in. Shift + drag to pan',
